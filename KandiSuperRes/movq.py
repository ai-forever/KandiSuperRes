    ...
    def tiled_encode2(self, x):
        ...
        for i in range(0, x.shape[2], overlap_size):
            for j in range(0, x.shape[3], overlap_size):
                ...
                tiles.append(tile)
            n += 1
        tiles = torch.stack(tiles).squeeze()
        
        # Encode tiles.
        tiles = torch.cat([self.encode(tile) for tile in tiles.chunk(tiles.shape[0]//4)])
        rows = list(list(tile.chunk(tiles.shape[0]//n)) for tile in tiles.chunk(n))
    ...
    def tiled_decode2(self, z):
        ...
        for i in tqdm(range(0, z.shape[2], overlap_size)):
            for j in range(0, z.shape[3], overlap_size):
                if j + self.tile_latent_min_size > z.shape[3]:
                    h_end = z.shape[3]
                    h_start = z.shape[3] - self.tile_latent_min_size
                    blend_extent_new_j = abs(h_start - j)*self.scale_factor + blend_extent 
                else:
                    h_end = j + self.tile_latent_min_size
                    h_start = j
                    
                if i + self.tile_latent_min_size > z.shape[2]:
                    w_end = z.shape[2]
                    w_start = z.shape[2] - self.tile_latent_min_size
                    blend_extent_new_i = abs(w_start - i)*self.scale_factor + blend_extent
                else:
                    w_end = i + self.tile_latent_min_size
                    w_start = i
        ...
        # Decode tiles.
        tiles = torch.cat([self.decode(tile) for tile in tiles.chunk(tiles.shape[0]//2)])
        rows = list(list(tile.chunk(tiles.shape[0]//n)) for tile in tiles.chunk(n))
        ...
        for i, row in enumerate(rows):
            ...
            for j, tile in enumerate(row):
                # blend the above tile and the left tile
                # to the current tile and add the current tile to the result row
                row_limit_new_i, row_limit_new_j = None, None
                if i > 0:
                    blend_extent_new = blend_extent_new_i if i+1 == n else blend_extent
                    tile = self.blend_v(rows[i - 1][j], tile, blend_extent_new)
                    row_limit_new_i = self.tile_sample_min_size - blend_extent_new
                if j > 0:
                    blend_extent_new = blend_extent_new_j if j+1 == n else blend_extent
                    tile = self.blend_h(row[j - 1], tile, blend_extent_new)
                    row_limit_new_j = self.tile_sample_min_size - blend_extent_new
                    
                row_limit_new_i = row_limit if row_limit_new_i is None else row_limit_new_i
                row_limit_new_j = row_limit if row_limit_new_j is None else row_limit_new_j
                result_row.append(tile[ :, :, :row_limit_new_i, :row_limit_new_j])
            result_rows.append(torch.cat(result_row, dim=3))
